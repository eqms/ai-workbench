//! CLI integration tests for the ai-workbench binary.
//!
...
fn workbench_binary() -> &'static str {
    env!("CARGO_BIN_EXE_ai-workbench")
}
...
fn help_prints_usage_and_exits_zero() {
    let output = Command::new(workbench_binary())
        .arg("--help")
        .output()
        .expect("failed to invoke ai-workbench --help");
...
fn version_prints_semver_and_exits_zero() {
    let output = Command::new(workbench_binary())
        .arg("--version")
        .output()
        .expect("failed to invoke ai-workbench --version");
