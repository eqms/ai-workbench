...
//! - [`global`] — keys that fire regardless of active pane (F12/F10/F7/F9/F11, Ctrl+P/O/X, Shift+F2, F8, Ctrl+Shift+W)
...
impl App {
    ...
    pub(super) fn focus_claude(&mut self) {
        ...
        // Startup prefixes are Claude slash-commands — irrelevant for other backends.
        if self.backend.supports_claude_flags()
            && !self.claude_startup.shown_this_session
