...
//! F11 universal paste, Ctrl+P/O/X pickers, Shift+F2 external editor,
...
impl App {
    pub(super) fn handle_global_shortcut(&mut self, key: KeyEvent) -> bool {
        ...
        // Shift+F2: Open selected file in external GUI editor. Was plain
        // Ctrl+E until Claude Code reserved that combo for its own use;
        // briefly rebound to Ctrl+Alt+E, but macOS terminals (Terminal.app,
        // iTerm2, Kitty) don't forward the Option/Alt modifier for
        // Ctrl+Option+<letter> under default keyboard settings — Option is
        // consumed by the OS's dead-key/composition input method before it
        // reaches the terminal, so that combo was a silent no-op for most
        // users. Then briefly rebound to Ctrl+F2, but macOS reserves
        // Ctrl+F2..Ctrl+F8 system-wide for Full Keyboard Access focus
        // navigation (System Settings > Keyboard > Keyboard Shortcuts >
        // Keyboard), which intercepts the key before it ever reaches the
        // terminal. Shift+F-key combos are not claimed by any macOS system
        // shortcut and use the same reliable CSI-encoded F-key + modifier
        // pattern already proven by the working Shift+F9 shortcut above, so
        // we use Shift+F2 instead.
        if key.code == KeyCode::F(2) && key.modifiers.contains(KeyModifiers::SHIFT) {
