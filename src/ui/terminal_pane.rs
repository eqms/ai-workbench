...
pub fn render(f: &mut Frame, area: Rect, pane_id: PaneId, app: &App) {
    ...
    let title = match pane_id {
        PaneId::Claude => app.backend.pane_title(),
    ...
    if let Some(error) = pane_error {
        let error_title = match pane_id {
            PaneId::Claude => format!("{} CLI Error", app.backend.short_label()),
            PaneId::LazyGit => "LazyGit Error".to_string(),
            PaneId::Terminal => "Shell Error".to_string(),
            _ => "PTY Error".to_string(),
        };
