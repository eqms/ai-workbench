...
/// Resolve a requested (relative) path against `base`, rejecting path traversal.
///
/// Returns the canonicalized target only if it stays within the canonicalized
/// `base` directory; returns `None` for a traversal attempt (`..`, absolute
/// re-root, escaping symlink) or a non-existent target. This is the security
/// boundary for `#image("...")` references coming from untrusted markdown.
fn resolve_within_base(base: &Path, requested: &Path) -> Option<PathBuf> {
...
impl typst::World for WorkbenchWorld {
    ...
    fn file(&self, id: FileId) -> FileResult<Bytes> {
        ...
        // Path-traversal guard: a crafted `..` path (e.g. from an untrusted
        // markdown image reference) must not read files outside the document's
        // directory. Mirrors the guard in `browser/markdown.rs`.
        match resolve_within_base(&self.source_dir, rel) {
            Some(canonical) => match std::fs::read(&canonical) {
        ...
        for event in parser {
            match event {
                ...
                Event::End(TagEnd::CodeBlock) => {
                    ...
                    // Pick a fence longer than any backtick run inside the code so an
                    // embedded ``` cannot close the block early and inject Typst markup.
                    let fence = "`".repeat(longest_backtick_run(code).max(2) + 1);
                    // The language tag becomes a raw-block attribute; strip anything
                    // that is not a plain identifier so it cannot carry markup.
                    let lang = lang.map(|l| {
                        l.chars()
                            .take_while(|c| {
                ...
                Event::Start(Tag::Link { dest_url, .. }) => {
                    ...
                    if let Some(anchor) = url.strip_prefix('#') {
                        ...
                        r.push_to_active(&format!("#link(\"{}\")[", typst_str_escape(&url)));
                    }
                }
                ...
                Event::Start(Tag::Image { dest_url, .. }) => {
                    ...
                    if url.starts_with("http://") || url.starts_with("https://") {
                        ...
                        // URL goes into a string literal (str-escape) and into content
                        // display text (content-escape) — both must be neutralized.
                        r.out.push_str(&format!(
                            "#text(fill: rgb(\"#666666\"), size: 9pt)[Image: #link(\"{}\")[{}]]",
                            typst_str_escape(&url),
                            typst_escape(&url)
                        ));
                    } else {
                        r.out
                            .push_str(&format!("#image(\"{}\")", typst_str_escape(&url)));
                    }
                }
...
/// Escape a string for safe interpolation into a Typst **string literal** (`"..."`).
///
/// Unlike [`typst_escape`] (which escapes content-mode markup), this only needs to
/// neutralize the two characters that are significant inside a Typst string literal:
/// backslash and double-quote. Newlines are stripped because a raw newline cannot
/// appear inside a Typst string literal. This is the security boundary for
/// untrusted markdown fields (link/image URLs) that are emitted as
/// `#link("...")` / `#image("...")` — without it, a `"` in a URL breaks out of the
/// string and injects arbitrary Typst code (which can read local files).
fn typst_str_escape(s: &str) -> String {
    ...
    for c in s.chars() {
        match c {
            '\\' => result.push_str("\\\\"),
            '"' => result.push_str("\\\""),
            '\n' | '\r' => {} // drop: not representable in a string literal
            _ => result.push(c),
        }
    }
    result
...
/// Length of the longest consecutive run of backtick characters in `s`.
///
/// Used to pick a raw-block fence longer than any backtick run inside the code, so
/// embedded ` ``` ` sequences cannot prematurely close the fence and inject markup.
...
mod tests {
    ...
    // --- Security: Typst-string injection & path traversal (T-typst-inj) ---

    #[test]
    fn test_typst_str_escape_neutralizes_quote_and_backslash() {
        assert_eq!(typst_str_escape("plain"), "plain");
        assert_eq!(typst_str_escape("a\"b"), "a\\\"b");
        assert_eq!(typst_str_escape("a\\b"), "a\\\\b");
        // Newlines cannot live in a string literal → dropped.
        assert_eq!(typst_str_escape("a\nb"), "ab");
    }

    #[test]
    fn test_link_url_with_quote_cannot_break_out() {
        // A crafted link URL containing a double-quote must not escape the Typst
        // string literal (which would allow arbitrary Typst code / file reads).
        let doc = DocumentConfig::default();
        let md = "[x](a\")#read(\"/etc/passwd\")//)";
        let result = TypstRenderer::render(md, &doc);
        // The raw breakout sequence must not appear; the quote must be escaped.
        assert!(
            !result.contains("#link(\"a\")#read"),
            "link URL broke out of the string literal: {result}"
        );
        assert!(result.contains("\\\""), "quote was not escaped: {result}");
    }

    #[test]
    fn test_local_image_url_with_quote_cannot_break_out() {
        let doc = DocumentConfig::default();
        let md = "![alt](img.png\")#read(\"/etc/passwd\")//)";
        let result = TypstRenderer::render(md, &doc);
        assert!(
            !result.contains("#image(\"img.png\")#read"),
            "image URL broke out of the string literal: {result}"
        );
    }

    #[test]
    fn test_code_fence_with_embedded_backticks_uses_longer_fence() {
        // A code block whose content contains a ``` sequence must be wrapped in a
        // longer fence so the embedded backticks cannot close the block early.
        let doc = DocumentConfig::default();
        let md = "````\nhas ``` inside\n````";
        let result = TypstRenderer::render(md, &doc);
        // Fence must be at least 4 backticks (longer than the internal run of 3).
        assert!(
            result.contains("````"),
            "expected a >=4 backtick fence, got: {result}"
        );
    }
    ...
    #[test]
    fn test_resolve_within_base_accepts_inside_and_rejects_traversal() {
        let tmp = tempfile::tempdir().unwrap();
        let base = tmp.path();
        // A file inside the base dir resolves.
        std::fs::write(base.join("inside.txt"), b"ok").unwrap();
        assert!(resolve_within_base(base, Path::new("inside.txt")).is_some());

        // A secret file OUTSIDE the base dir must be rejected via `..` traversal.
        let outside = tmp.path().parent().unwrap().join("wb_secret_test.txt");
        std::fs::write(&outside, b"secret").unwrap();
        let payload = format!("../{}", outside.file_name().unwrap().to_string_lossy());
        assert!(
            resolve_within_base(base, Path::new(&payload)).is_none(),
            "path traversal to {payload} was NOT rejected"
        );
