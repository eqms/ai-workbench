...
<p align="center">
  <img src="docs/Claude_Workbench.png" alt="AI Workbench - Rust-based TUI Multiplexer for AI-Assisted Development" width="800">
</p>
...
<p align="center">
  <img src="docs/ai-workbench-tui.png" alt="AI Workbench - Core Features & Integrated Workflow" width="900">
</p>
...
- **External editor shortcut rebound to Shift+F2 (was Ctrl+Alt+E).** The v1.1.1 rebind to Ctrl+Alt+E turned out to be a silent no-op on macOS terminals under default keyboard settings (Terminal.app, iTerm2, Kitty) — Option is consumed by the OS's dead-key/composition input method and never reaches the terminal as an Alt modifier when Ctrl is also held. An interim Ctrl+F2 rebind also failed: macOS reserves Ctrl+F2 through Ctrl+F8 system-wide for Full Keyboard Access focus navigation, so the key never reached the terminal at all. Shift+F2 is not claimed by any macOS system shortcut and uses the same reliable F-key + modifier encoding already proven by the working Shift+F9 shortcut, while still avoiding the original collision with Claude Code's own Ctrl+E binding.

### What's New in v1.1.2

- **Security hardening release.** The Markdown browser preview no longer renders raw HTML/`<script>` embedded in `.md` files — it is shown as escaped text instead (Markdown-syntax images and links are unaffected). The system file opener now canonicalizes paths so filenames starting with `-` cannot be misread as CLI flags. The CI/release pipeline runs with least-privilege permissions and all GitHub Actions are pinned to exact commit SHAs. See [SECURITY-NOTES.md](SECURITY-NOTES.md) for the full audit trail.
...
- **`--clipboard-diag` CLI** — `ai-workbench --clipboard-diag` prints the active strategy, helper paths, relevant environment variables (`DISPLAY`, `WAYLAND_DISPLAY`, `XRDP_SESSION`, `XDG_SESSION_TYPE`, `SSH_TTY`) and runs a copy/paste roundtrip test.
...
<p align="center">
  <img src="docs/ai-workbench-tui_de.png" alt="AI Workbench - Kern-Features & Integrierter Workflow" width="900">
</p>
...
- **Shortcut für externen Editor auf Shift+F2 verschoben (vorher Ctrl+Alt+E).** Die v1.1.1-Umstellung auf Ctrl+Alt+E erwies sich unter Standard-Tastatureinstellungen auf macOS-Terminals (Terminal.app, iTerm2, Kitty) als stiller Fehlschlag — die Option-Taste wird von der Zusammensetzungs-/Tottasten-Eingabemethode des Betriebssystems abgefangen und erreicht das Terminal nie als Alt-Modifikator, wenn gleichzeitig Ctrl gedrückt wird. Auch die Zwischenlösung Ctrl+F2 scheiterte: macOS reserviert Ctrl+F2 bis Ctrl+F8 systemweit für die Fokus-Navigation der Vollständigen Tastatursteuerung, sodass die Taste das Terminal nie erreichte. Shift+F2 ist von keinem macOS-Systemshortcut belegt und nutzt dieselbe zuverlässige F-Tasten-plus-Modifikator-Kodierung, die bereits beim funktionierenden Shift+F9-Shortcut bewährt ist, und vermeidet weiterhin die ursprüngliche Kollision mit Claude Codes eigener Ctrl+E-Belegung.

### Neu in v1.1.2

- **Security-Hardening-Release.** Die Markdown-Browser-Vorschau rendert kein rohes HTML/`<script>` mehr, das direkt in `.md`-Dateien eingebettet ist — es wird stattdessen als escapter Text angezeigt (Markdown-Syntax für Bilder und Links ist nicht betroffen). Der System-Dateiöffner kanonisiert Pfade, sodass Dateinamen mit führendem `-` nicht mehr als CLI-Flags fehlinterpretiert werden können. Die CI/Release-Pipeline läuft mit Least-Privilege-Berechtigungen und alle GitHub Actions sind auf exakte Commit-SHAs gepinnt. Details im vollständigen Audit-Trail: [SECURITY-NOTES.md](SECURITY-NOTES.md).
...
- **`--clipboard-diag` CLI** — `ai-workbench --clipboard-diag` druckt die aktive Strategie, Helper-Pfade, relevante Umgebungsvariablen (`DISPLAY`, `WAYLAND_DISPLAY`, `XRDP_SESSION`, `XDG_SESSION_TYPE`, `SSH_TTY`) und führt einen Copy/Paste-Roundtrip aus.
