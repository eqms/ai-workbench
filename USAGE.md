...
**Config** (`~/.config/ai-workbench/config.yaml`, under `ui:`):
...
```bash
ai-workbench --open-diag
```

Reports SSH state, the detected terminal capability, the raw environment
...
| Shift+F2 | In externem Editor öffnen (Vorschau-Datei wenn Vorschau aktiv, sonst Dateibrowser-Auswahl; konfigurierbar in Einstellungen F8) |
...
**Konfiguration** (`~/.config/ai-workbench/config.yaml`, unter `ui:`):
